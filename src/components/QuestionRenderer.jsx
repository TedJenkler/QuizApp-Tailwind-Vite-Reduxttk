...
function QuestionRenderer( {selectedTopic, selectedQ, setSelectedQ, choice, setChoice, points, setPoints} ) {
  ...
  return (
    <div className=''>
        ...
        <div className={`${darkMode === "dark" ? "w-full h-4 bg-navy flex items-center mb-10 rounded-full" : "w-full h-4 bg-white flex items-center mb-10 rounded-full"}`}>
            <div style={{ width: `${selectedQ}0%` }} className={`${darkMode === "dark" ? "bg-purple h-2 mx-1 rounded-full" : "bg-purple h-2 mx-1 rounded-full"}`}></div>
        </div>
        <button onClick={(e) => {setChoice(state.options[0])}} className={`${darkMode === "dark" ? "flex items-center mb-3 p-3 rounded-xl w-full focus:border-purple focus:border-4 bg-navy shadow-2xl text-white" : "flex items-center mb-3 bg-pure-white p-3 rounded-xl w-full focus:border-purple focus:border-4"}`}><img className='mr-3' src={a} alt='a' />{state.options[0]}</button>
